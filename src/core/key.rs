...
pub fn encrypt_data(key: &[u8], plaintext: &[u8]) -> (Vec<u8>, Vec<u8>) {
    let mut noncebuf: GenericArray<u8, <XChaCha20Poly1305 as Aead>::NonceSize> =
...
pub fn decrypt_data(key: &[u8], encrypted: &[u8]) -> Option<Vec<u8>> {
    let nonce_size = <XChaCha20Poly1305 as Aead>::NonceSize::to_usize();
...
pub fn derive_phase_key(side: &str, key: &Key, phase: &Phase) -> Vec<u8> {
    ...
    let length = <XChaCha20Poly1305 as NewAead>::KeySize::to_usize();
